...
from nelo2_logging_handler.nelo2_logging_handler import AsyncNelo2LoggingHandler, Nelo2LoggingHandler
...
def test_async_nelo2_handler():
    ...
    nelo2_handler = AsyncNelo2LoggingHandler(project_name=os.environ['PROJECT_NAME'], project_version='0.1',
    ...
    for i in range(10):
        ...
        logger.debug(f"nelo2 handler info: {datetime.now()}")
        sleep(2)


def test_sync_nelo2_handler():
    root_logger = logging.getLogger("test_logger")
    root_logger.setLevel("DEBUG")
    root_formatter = logging.Formatter("%(asctime)s - %(name)s:%(lineno)d %(funcName)20s - %(levelname)s - %(message)s")
